...
public class ImplTmplGenerator {
	...
	private void write(SetFile setFile) throws Exception {
		...
		for(Card card:setFile.cards){
			...
			// rarity
			rarity = card.rarity.toUpperCase(Locale.ENGLISH);
			if (rarity.equals("RARE HOLO")){
				rarity="HOLORARE";
			} else if (rarity.equals("ULTRA RARE")){
				rarity="ULTRARARE";
			}
			cardTypeSet.add(card.superType);
			cardTypeSet.addAll(card.subTypes);
			if(card.superType.equalsIgnoreCase("POKEMON")) {
				//hp
				...
				if (card.types != null) {
					if (card.types.size() == 1) {
						typesCombined = card.types.get(0);
					} else {
						typesCombined = card.types.toString();
					}
					for (String _type : card.types) {
						cardTypeSet.add("_" + typesMap.getOrDefault(_type, _type) .toUpperCase(Locale.ENGLISH)+ "_");
					}
				}
				if (card.subTypes.contains("BABY")) {
